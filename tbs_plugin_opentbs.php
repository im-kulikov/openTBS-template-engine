...
/* OpenTBS version 1.3.1, on 2010-07-01
...
class clsOpenTBS extends clsTbsZip {
	...
	function BeforeShow(&$Render, $File='') {
		...
		if (isset($this->OpenXmlRid))   $this->OpenXML_RidCommit();      // Commit special OpenXML features if any
		if (isset($this->OpenDocManif)) $this->OpenDoc_ManifestCommit(); // Commit special OpenDocument features if any
...
*/		
	...
	function OpenDoc_ManifestCommit() {
		...
		$idx = $this->FileGetIdx('META-INF/manifest.xml');
		if ($idx===false) return;
